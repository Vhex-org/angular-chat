...
export class UserService {
    ...
    login(data: FormValues) {
        ...
        const pHeaders = new HttpHeaders({ 'Content-Type': 'application/json' });
        const body = JSON.stringify(data);
        this.http.post<string>(`${Config.serverUrl}/${Config.user.auth}`, body, { headers: pHeaders }).subscribe({
            error: err => console.log(err),
            next: (resp: any) => {
                const gHeaders = new HttpHeaders({ Authorization: `Bearer ${resp}` });
                this.http.get<any>(`${Config.serverUrl}/${Config.user.profile}`, { headers: gHeaders }).subscribe({
                    error: err => console.log(err),
                    next: (userInfo: any )=> {
